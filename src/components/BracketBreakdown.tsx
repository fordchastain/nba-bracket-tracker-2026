import type { ScoredEntry, ActualResults, ActualMatchup, Matchup } from '../../types/bracket';
...
function MatchupRow({ matchup, actualWinner }: { matchup: Matchup; actualWinner: string | null | undefined }) {
  ...
  return (
    <div className={`matchup-row${correct ? ' correct' : incorrect ? ' incorrect' : ' pending'}`}>
      <span className="teams">{matchup.team1} vs {matchup.team2}</span>
      <span className="pick">{matchup.winner}</span>
      {pending && <span className="tbd">TBD</span>}
      {incorrect && <span className="actual-result">Actual: {actualWinner}</span>}
    </div>
  );
}
...
export default function BracketBreakdown({ entry, actual }: Props) {
  ...
  const finalsCorrect = finalsChamp != null && bracket.finals.champion === finalsChamp;
  const finalsIncorrect = finalsChamp != null && bracket.finals.champion !== finalsChamp;
    ...
    <div className="breakdown">
      ...
      <div className="breakdown-columns">
        <div className="breakdown-section">
          <h4>East — First Round <em>(1pt)</em></h4>
          {bracket.east.first.map(m => (
            <MatchupRow key={matchupKey(m.team1, m.team2)} matchup={m}
              actualWinner={eastFirstMap.get(matchupKey(m.team1, m.team2))} />
          ))}
          <h4>East — Second Round <em>(2pts)</em></h4>
          {bracket.east.second.map(m => (
            <MatchupRow key={matchupKey(m.team1, m.team2)} matchup={m}
              actualWinner={eastSecondMap.get(matchupKey(m.team1, m.team2))} />
          ))}
          <h4>East — Conference Finals <em>(4pts)</em></h4>
          <MatchupRow matchup={bracket.east.finals} actualWinner={actual.east.finals.winner} />
        </div>

        <div className="breakdown-section">
          <h4>West — First Round <em>(1pt)</em></h4>
          {bracket.west.first.map(m => (
            <MatchupRow key={matchupKey(m.team1, m.team2)} matchup={m}
              actualWinner={westFirstMap.get(matchupKey(m.team1, m.team2))} />
          ))}
          <h4>West — Second Round <em>(2pts)</em></h4>
          {bracket.west.second.map(m => (
            <MatchupRow key={matchupKey(m.team1, m.team2)} matchup={m}
              actualWinner={westSecondMap.get(matchupKey(m.team1, m.team2))} />
          ))}
          <h4>West — Conference Finals <em>(4pts)</em></h4>
          <MatchupRow matchup={bracket.west.finals} actualWinner={actual.west.finals.winner} />
        </div>
      </div>

      <div className="breakdown-section finals-section">
        <h4>NBA Finals Champion <em>(8pts)</em></h4>
        <div className={`matchup-row${finalsCorrect ? ' correct' : finalsIncorrect ? ' incorrect' : ' pending'}`}>
          <span className="pick">{bracket.finals.champion}</span>
          {finalsChamp == null && <span className="tbd">TBD</span>}
          {finalsIncorrect && <span className="actual-result">Actual: {finalsChamp}</span>}
        </div>
      </div>
    </div>
  );
